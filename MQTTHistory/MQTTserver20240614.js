...
async function writeClient(Devices) {
  ...
  try {
    ...
    // write file
  await fs.promises.writeFile(pathtree, jsonString, 'utf8');
  } catch (err) {
    if (err.code === 'ELOCKED') {
      console.error("Error: The file is already locked by another process");
...
aedes.on("publish", (packet, client) => {
  ...
  if(packet.topic=="state/info"){
    ...
    if(InfoDetail.length==0){
      console.log("InfoDetail is empty");
      return;
    }
...
//data send
function dataSend(jsonString,type){

  if(type=='Device'){
    socket.emit('client data',jsonString);
  }else if(type=='topics'){
    socket.emit('client topics',jsonString);
  } 
}
